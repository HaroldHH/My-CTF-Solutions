...
# Payload : AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA0000beef
# Flag : COMPFEST13{th1s_is_h0w_bufFer_overfloW_ActuaLLy_woRk5_a3925560}
#
...
def EVAL(ins):
    ...
    if ins[:4] == "CALL":
        ...
        func_called = ins[5:]
        print("[*] Called function : " + func_called)
        EIP_hex = hex(EIP)[2:].rjust(8,'0')
        print("[*] EIP from CALL : " + str(EIP_hex) + " / " + str(EIP))
        ESP = ESP - 8
        print("[*] ESP from CALL : " + str(hex(ESP)[2:].rjust(8,'0')))
        write_memory(ESP, EIP_hex)
        EIP = ADDRESS[func_called]
        print("[*] EIP after CALL : " + str(hex(EIP)[2:].rjust(8,'0')))
    elif ins[:4] == "PUSH":
        var_name = ins[5:]
...
def run():
    ...
    while(EIP != 10 ** 6):
        ...
        print("[*] read_memory performed, return " + str(ins) + " and " + str(hex(EIP)[2:].rjust(8,'0')))
        EVAL(ins)
